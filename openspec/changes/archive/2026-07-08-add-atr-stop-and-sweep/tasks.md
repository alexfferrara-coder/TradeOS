...
- [x] 1.1 Add fixtures under `test/fixtures/risk-rules/`: valid ATR config
...
- [x] 1.2 Extend `src/risk/loadRiskRules.js`: read `stop_method` (default
      `'channel'` when absent), validate `atr_period`/`atr_multiple` only
      when `stop_method === 'atr'`, return the new fields alongside the
      existing three.
- [x] 1.3 Add/extend tests in `test/loadRiskRules.test.js` covering every
      new fixture from 1.1 plus the existing `max_risk_per_trade <= 0.1`
      ceiling still enforced when `stop_method` is `atr`.
- [x] 1.4 Run `node --test` and confirm all existing + new loader tests
      pass before moving on.

## 2. ATR calculation module

- [x] 2.1 Create `backtest/atr.js` exporting `atrSeries(bars, atr_period)`:
...
- [x] 2.2 Add unit tests for `atr.js` against a small hand-computed OHLC
...
- [x] 3.1 In `backtest/backtest.js`, precompute `atrSeries` per symbol
      (alongside `barsBySymbol`) when `rules.stop_method === 'atr'`.
- [x] 3.2 Branch entry-time `stopLevel` computation on `rules.stop_method`:
      `channel` keeps `lowestClose(bars, i, exitLookback)`; `atr` uses
      `close - rules.atr_multiple * atrSeries[i]`.
- [x] 3.3 Extend the entry warm-up guard so entries are skipped until both
      `entryLookback` and (in atr mode) `atr_period` bars of history exist.
- [x] 3.4 Confirm the exit loop (channel-low trailing check + fixed
...
- [x] 3.5 Extend `test/backtest.integration.test.js` with an atr-mode run
...
- [x] 4.1 Update `backtest/stats.js`: change `summarize(trades)` to
...
- [x] 4.2 Add tests for `summarize()` covering: empty trade list (drawdown
...
- [x] 5.5 Add `backtest/README.md` documentation for `node backtest/sweep.js`
      alongside the existing `run.js` docs, including the `stop_method: atr`
      precondition.

## 6. Final verification

- [x] 6.1 Run `node --test` — full suite green.
- [x] 6.2 Run `node backtest/run.js` against the live rules file (which
...
- [x] 6.3 Run `node backtest/sweep.js` and confirm the comparison table and
