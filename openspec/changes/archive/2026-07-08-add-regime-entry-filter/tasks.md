...
- [x] 1.1 Add `smaClose(bars, i, period)` to `backtest/strategy.js` — mean of
      closes over `[i-period, i-1]` (prior N days, today excluded), matching
      the `highestClose`/`lowestClose` convention.
- [x] 1.2 Add unit tests for `smaClose` against a small hand-computed series
      (verify the window boundaries and the excluded-today convention).

## 2. Config

- [x] 2.1 Add `strategy: { regimeFilter: { enabled: false, smaPeriod: 200 } }`
...
- [x] 3.1 Change `runBacktest(barsBySymbol, rules)` to
      `runBacktest(barsBySymbol, rules, strategy = config.strategy)`.
- [x] 3.2 In the entries loop, when `strategy.regimeFilter.enabled`, skip an
      otherwise-valid breakout when `close <= smaClose(bars, i, smaPeriod)`.
- [x] 3.3 Extend the entry warm-up guard so that, when the filter is enabled,
      entries are skipped until `i >= smaPeriod`.
- [x] 3.4 Extend `test/backtest.integration.test.js`: a filter-on case where a
      ...
      pre-change trades.
- [x] 3.5 Run `node --test` and confirm all existing + new tests pass.

## 4. Regime sweep

- [x] 4.1 Create `backtest/regime-sweep.js`: load rules first and fail closed
...
- [x] 4.2 Print a comparison table (config label, trades, win rate, expectancy
      ...
      pattern.
- [x] 4.3 Document the regime filter and `node backtest/regime-sweep.js` in
      `backtest/README.md`.

## 5. Verification

- [x] 5.1 Run `node --test` — full suite green.
- [x] 5.2 Run `node backtest/regime-sweep.js` and record the filter-off vs
