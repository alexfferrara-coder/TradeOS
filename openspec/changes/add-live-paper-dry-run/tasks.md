...
- [ ] 1.1 Create `backtest/decide.js` with `decideExit({ bars, i, position,
...
- [ ] 1.2 Rewire `backtest/backtest.js` to call `decideExit`/`decideEntry`
      instead of its inline checks, preserving the exits-then-entries phasing.
- [ ] 1.3 Run `node --test` and confirm all 43 existing tests pass unchanged
      (exact-trade + no-look-ahead assertions are the refactor guardrail).
- [ ] 1.4 Add direct unit tests for `decideEntry`/`decideExit` covering a
      breakout taken, a below-SMA breakout filtered, and an exit trigger.

## 2. Alpaca paper broker (read-only)

- [ ] 2.1 Create `backtest/broker.js` exporting a factory that returns
...
- [ ] 2.2 Unit-test `broker.js` with a fake fetch (assert paper endpoint,
      headers, and bar normalization).

## 3. Local strategy-state store

- [ ] 3.1 Create `backtest/state.js`: load/save a gitignored JSON map of
      symbol → `{ stopLevel, riskPerShare, entryDate }`, with a reconcile
      helper that flags broker positions lacking local state.
- [ ] 3.2 Unit-test `state.js` load/save/reconcile against temp files.
- [ ] 3.3 Add the state file path to `.gitignore`.

## 4. Live dry-run entrypoint

- [ ] 4.1 Create `backtest/live.js`: load rules first and fail closed; take an
...
- [ ] 4.2 Test `live.js` orchestration with a mock broker: a flat account
      (entry-only report) and a seeded-position account (exit/hold report),
      asserting nothing is ever submitted.

## 5. Docs and verification

- [ ] 5.1 Document `node backtest/live.js` in `backtest/README.md` — dry-run,
      paper-only, no-submit, decides on the last completed bar.
- [ ] 5.2 Run `node --test` — full suite green.
- [ ] 5.3 Run `node backtest/live.js` against the live paper account and
