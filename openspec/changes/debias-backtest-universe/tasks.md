...
- [x] 1.1 Add `BA, INTC, WBA, PYPL, NFLX, DIS, F, META, ROKU, T, NKE` to
      `config.symbols` in `backtest/config.js` (5 → 16), leaving the existing
      five entries in place.

## 2. Documentation

- [x] 2.1 Update the survivorship-bias note in `backtest/README.md` to
...
- [x] 3.1 Run `node backtest/run.js` and confirm all 16 symbols fetch with
      ...
      the new symbols).
- [x] 3.2 Run `node backtest/sweep.js` and record the honest `atr_multiple`
...
- [x] 4.1 Run `node --test` — full suite still green (no logic changed, but
